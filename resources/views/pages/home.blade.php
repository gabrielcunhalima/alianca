...
@section('title', 'Aliança Consultoria — Engenharia, Infraestrutura e Sustentabilidade')
@section('meta_description', 'Consultoria especializada em engenharia, portos, rodovias, ferrovias, mobilidade urbana, meio ambiente, captação de recursos e licitações públicas. Atuação nacional desde 2012.')
    ...
    <div class="relative max-w-7xl mx-auto px-6 lg:px-20 py-32 w-full">
        <div class="max-w-5xl">
            ...
            <div class="flex flex-col sm:flex-row gap-4 mb-20 reveal-4">
                <a href="{{ route('contato') }}" class="btn-accent text-base">
                ...
                <a href="{{ route('servicos') }}" class="btn-outline-light text-base">
                    Ver nossos serviços
                </a>
            </div>
            ...
            <!-- <div class="grid grid-cols-2 md:grid-cols-4 gap-0 border rounded-sm reveal-fade"
                ...
                <div class="px-6 py-5 {{ $i > 0 ? 'border-l' : '' }}"
                     style="{{ $i > 0 ? 'border-color: rgba(80,110,200,0.12);' : '' }}">
                    <p class="font-display font-extrabold text-3xl md:text-4xl mb-1" style="color: #EEEEEE;">{{ $s['valor'] }}</p>
                    <p class="text-xs uppercase tracking-widest" style="color: rgba(238,238,238,0.35);">{{ $s['label'] }}</p>
                </div>
                @endforeach
            </div> -->
...
<section class="py-28" style="background-color: #080b15;">
    <div class="max-w-7xl mx-auto px-6 lg:px-20">
        ...
        <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-16">
            ...
            <a href="{{ route('servicos') }}" class="btn-outline-light text-sm shrink-0">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px" style="background-color: rgba(80,110,200,0.08);">
            ...
            <a href="{{ route('servicos') }}#{{ $s['anchor'] }}"
               class="group card-premium p-8 block rounded-none"
               style="border-radius: 0; border: none;">
                <div class="flex items-start justify-between mb-6">
                    <span class="font-display font-bold text-xs tabular-nums" style="color: rgba(218,18,18,0.5);">{{ $s['num'] }}</span>
                ...
                <h3 class="font-display font-bold text-lg mb-3" style="color: #EEEEEE;">{{ $s['titulo'] }}</h3>
                <p class="text-sm leading-relaxed mb-6" style="color: rgba(238,238,238,0.5);">{{ $s['desc'] }}</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            ...
            <div>
                ...
                <ul class="space-y-5 mb-10">
                    ...
                    <li class="flex items-start gap-4">
                        ...
                        <span class="text-base" style="color: rgba(238,238,238,0.75);">{{ $d }}</span>
                    </li>
                    @endforeach
                </ul>
                <a href="{{ route('sobre') }}" class="btn-outline-light text-sm">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="rounded-sm p-7 card-premium">
                    <p class="font-display font-extrabold text-4xl mb-1" style="color: #EEEEEE;">{{ $n['valor'] }}</p>
                    <p class="text-sm font-medium mb-1" style="color: rgba(238,238,238,0.6);">{{ $n['label'] }}</p>
                    <p class="text-xs" style="color: rgba(238,238,238,0.3);">{{ $n['sub'] }}</p>
                </div>
                @endforeach
            </div>
...
<section class="py-14 border-y overflow-hidden" style="background-color: #080b15; border-color: rgba(80,110,200,0.10);">
    <div class="max-w-7xl mx-auto px-6 lg:px-20 mb-8 flex items-center justify-between">
        ...
        <a href="{{ route('clientes') }}" class="text-xs font-semibold uppercase tracking-widest transition-colors hidden md:inline-flex items-center gap-2" style="color: rgba(218,18,18,0.7);" onmouseover="this.style.color='#DA1212'" onmouseout="this.style.color='rgba(218,18,18,0.7)'">
            Ver cases
    ...
    <div class="relative" style="-webkit-mask-image: linear-gradient(to right, transparent 0%, black 10%, black 90%, transparent 100%); mask-image: linear-gradient(to right, transparent 0%, black 10%, black 90%, transparent 100%);">
        <div class="marquee-track">
            ...
            @endphp
            @foreach([$clientes, $clientes] as $grupo)
                @foreach($grupo as $cliente)
                <div class="flex items-center gap-16 px-10 shrink-0">
                    <span class="font-display font-bold text-sm md:text-base tracking-wider whitespace-nowrap" style="color: rgba(238,238,238,0.22);">{{ $cliente }}</span>
    ...
    <div class="relative max-w-7xl mx-auto px-6 lg:px-20">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            ...
            <div class="card-premium rounded-sm p-6">
                <p class="font-display font-bold text-xs tabular-nums mb-4" style="color: rgba(218,18,18,0.5);">{{ str_pad($i+1, 2, '0', STR_PAD_LEFT) }}</p>
                <h3 class="font-display font-bold text-base mb-2" style="color: #EEEEEE;">{{ $v['nome'] }}</h3>
                <p class="text-sm leading-relaxed" style="color: rgba(238,238,238,0.45);">{{ $v['desc'] }}</p>
            </div>
            @endforeach
        </div>
